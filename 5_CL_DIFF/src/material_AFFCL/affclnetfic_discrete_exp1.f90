...
! ================= KINETICS & ODE INTEGRATION (SIMPLE EXPLICIT) =================
    cb_i = cb(node_num)
    
    ! Calculate reaction rates based on the state at the start of the increment
    kon = Koff0 * Keq * exp(CHI * (1.0d0 - 2.0d0 * thetaf))
    koff_i = Koff0 * exp((fi * dx) / (kb * theta))
    
    ! Determine bound fraction and clamp to avoid division by zero
    thetab_i = cb_i / cbmax
    thetab_i = MIN(MAX(thetab_i, 1.0d-6), one - 1.0d-6)
    
    ! Calculate net reaction rate R_i for this direction
    R_i = kon * cfmax * (thetaf / (1.0d0 - thetaf)) &
        - koff_i * cbmax * (thetab_i / (1.0d0 - thetab_i))
    
    ! Update the bound concentration using the global increment size
    cb(node_num) = cb_i + dtime * R_i
    
    ! Ensure cb does not drop into negative or zero values
    cb(node_num) = MAX(cb(node_num), 1.0d-10)
